...
//Serial.print("keyboard bagan");
  ...
  if (handlebar > (wheelcenter+100)) { //handlebar turns right, turn kart right
...
//      Keyboard.write(turnRight); //press the key again
  ...
  else if (handlebar < (wheelcenter-100)) { //handlebar turns left, turn left
...
//      Keyboard.write(turnLeft); //press the key again
   ...
   if(handlebar <(wheelcenter+100) && handlebar >(wheelcenter-100)){
...
//delay(200);
  if (keyboardEnabled && keyIsYetToBePressed == true) {
    Keyboard.press(keyToBePressed); //output the button press as a keyboard command
    keyIsYetToBePressed = false;
  }
  Keyboard.release(keyToBePressed); 
 // debugData();
}
